...
def fit_model(
    model_data, X_train, y_train, date_double_weights=config["date_double_weights"]
):
...
def perform_kfold_cross_validation(
    ...
    for train_index, test_index in kf.split(X):
        ...
        # np.where always returns a tuple, so we need to extract the first element
...
def compare_average_to_model(results_constant, results_model):
...
def summarise_results_categories(results_per_categorical_feature):
